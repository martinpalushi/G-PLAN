...
(function () {
  ...
  const SAVED_MAJOR_KEY = "gplan.savedMajor";
  const SAVED_MAJOR_NAME_KEY = "gplan.savedMajorName";
  ...
  function recomputeTotalCredits() {
    let total = 0;
    planYears
      .querySelectorAll("li[data-credits]")
      .forEach(li => (total += +li.dataset.credits));
  ...
  function recomputeSemesterCredits(card) {
    ...
    card
      .querySelectorAll("li[data-credits]")
      .forEach(li => (sum += +li.dataset.credits));
    if (p) p.textContent = `Total Credits: ${sum}`;
  }

  // Counts the amount of courses and prevents it from surpassing 5
  const getSemesterCourseCount = card =>
    card.querySelectorAll("li[data-course-code]").length;

  function findCourseDataByCode(code) {
    if (!window.majorsData) return null;
    for (const majorKey in window.majorsData) {
      const major = window.majorsData[majorKey];
  ...
  function enhanceCurrentPlanDOM() {
    planYears
      .querySelectorAll("li[data-course-code]")
      .forEach(makeCourseDraggable);
    planYears
      .querySelectorAll(".semester-card")
      .forEach(enableSemesterDrop);
  }

  // Builds the plan for a given major slug
  function renderMajorPlan(slug) {
    if (!window.majorsData) {
      planMajorName.textContent =
    ...
    const major = window.majorsData[slug];
    if (!major) {
      planMajorName.textContent = "Unknown Major";
      planMajorDesc.textContent = "";
      planYears.innerHTML = "";
      planTotalCredits.textContent = "0";
      return;
    ...
    (major.years || []).forEach(year => {
      ...
      ["fall", "winter"].forEach(semName => {
        ...
        semDiv.className =
          "semester-card" + (semName === "fall" ? " gold" : "");
        const h3 = document.createElement("h3");
        h3.textContent =
          semName.charAt(0).toUpperCase() +
          semName.slice(1) +
          " Semester";
  ...
  document.querySelectorAll(".major-card[data-major]").forEach(card => {
    card.addEventListener("click", () => {
      ...
      // ✅ save selection for integrated minor plan
      localStorage.setItem(SAVED_MAJOR_KEY, slug);

      if (window.majorsData && window.majorsData[slug]) {
        localStorage.setItem(
          SAVED_MAJOR_NAME_KEY,
          window.majorsData[slug].name || slug
        );
      }
    });
  });

  // clicking course text -> open modal
  ...
  // checkboxes mark course complete & shift schedule
  ...
  // ✅ auto-load saved major if there is one
  const savedSlug = localStorage.getItem(SAVED_MAJOR_KEY);
  if (savedSlug && window.majorsData && window.majorsData[savedSlug]) {
    const savedCard = document.querySelector(
      `.major-card[data-major="${savedSlug}"]`
    );
